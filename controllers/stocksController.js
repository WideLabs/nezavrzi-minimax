...
const getStocks = async (req, res) => {
    const {authToken} = req
    const response = await apiGet(`${apiBaseUrl}/api/orgs/${orgId}/stocks`, authToken)
...
const getStockForItemById = async (req, res) => {
    const {authToken} = req
    const {id} = req.params
    const response = await apiGet(`${apiBaseUrl}/api/orgs/${orgId}/stocks/${id}`, authToken)
    if (response.statusCode !== httpStatusCodes.OK) {
        return res.status(response.statusCode).json(response)
    }
    return res.status(response.statusCode).json(
...
const getStockForItemByCode = async (req, res) => {
    ...
    // Get item by code
    const itemResponse = await apiGet(`${apiBaseUrl}/api/orgs/${orgId}/items/code(${code})`, authToken)
    if(itemResponse.statusCode !== httpStatusCodes.OK) return res.status(itemResponse.statusCode).json(itemResponse)
    // Get stock for item by ItemId
    const {ItemId} = itemResponse.data
    const response = await apiGet(`${apiBaseUrl}/api/orgs/${orgId}/stocks/${ItemId}`, authToken)
    if (response.statusCode !== httpStatusCodes.OK) {
        return res.status(response.statusCode).json(response)
    }
    return res.status(response.statusCode).json(
